...
async function outcomeBuilder(environment, object, organization, error, {outcomeDict, objectDict, impactNormsDict}, {
  ...
  let impactNorms;
  const mainObject = environment === 'fileUploading' ? outcomeDict[uri] : form.uri ? (await mainModel.findOne({_uri: form.uri}) || mainModel({}, {uri: form.uri})) : mainModel({});
  ...
  if (mainObject) {
    ...
    ret = await assignInvertValues(environment, config, object, mainModel, mainObject, {
        propertyName: 'indicators',
        internalKey: 'cids:hasIndicator'
      },
      objectDict, organization, {
        objectModel: GDBIndicatorModel,
        invertProperty: 'forOutcomes',
