...
export default function Organization_stakeholderOutcomes() {
  ...
  const columns = [
    {
      label: 'Legal Name',
      body: ({_uri, legalName}) => {
        return <Link colorWithHover to={`/stakeholderOutcomes/${encodeURIComponent(_uri)}`}>
          {legalName}
        </Link>
      },
      ...
      <DataTable
        ...
        customToolbar={
          <Chip
            ...
            onClick={() => navigate('/stakeholderOutcome/new')}
            color="primary"
            icon={<AddIcon/>}
            label="Add a new Stakeholder Outcome"
            variant="outlined"/>
        }
